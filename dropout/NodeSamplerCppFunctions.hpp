...
std::int32_t NodeSamplerCpp::get_num_weights_required() {
...
void NodeSamplerCpp::calc_output(
...
void NodeSamplerCpp::calc_delta(std::int32_t _batch_size) {
  ...
  if (this->NeuralNet->get_sample() == true) {
    ...
    for (auto node: this->hidden_nodes_fed_into_me_ptr) {
      ...
      thrust::for_each(
		       ...
		       DropoutFunctors::NodeSamplerCalcDelta() 
		       );
