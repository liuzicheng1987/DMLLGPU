...
void NeuralNetworkCpp::init_output_node(NeuralNetworkNodeCpp *_output_node) {
  ...
  if (
		  _output_node->node_number_ >= (std::int32_t)(this->nodes_.size())
...
void NeuralNetworkCpp::finalise(/*MPI_Comm comm, std::int32_t rank, std::int32_t size,*/float _weight_init_range) {
  ...
  if (std::any_of(this->nodes_.begin(), this->nodes_.end(), [](NeuralNetworkNodeCpp *node) {return node == nullptr;}))
  ...
  this->W_ = thrust::device_vector<float>(Whost.data(), Whost.data() + Whost.size());
...
void NeuralNetworkCpp::load_dense_targets(
  ...
  this->dense_targets_[_num_output_node] = 
...
void NeuralNetworkCpp::load_coo(
  ...
  for (std::int32_t batch_num = 0; batch_num<_num_batches; ++batch_num) {
    ...
    if (
    _num_output_node >= (std::int32_t)(this->sparse_targets_.size()) ||
      _num_output_node < 0
			 ) 
      throw std::invalid_argument("num_output_node out of bounds!");
    ...
    this->load_coo(
      this->sparse_targets_[_num_output_node],
      _Y_data,
      _Y_data_length,
      _Y_indices,
      _Y_indices_length,
    ...
    for (auto& data: this->dense_targets_) data.clear();
