void AdaDeltaCpp::min(/*MPI_Comm comm,*/
		     ...
		     std::vector<float> &_sum_gradients)
{
    ...
    //Initialise sum_updates_squared_
    this->sum_updates_squared_ = thrust::device_vector<float>(_W.size());

    thrust::fill(this->sum_updates_squared_.begin(),
		 this->sum_updates_squared_.end(),
		 0.f);		 
	...
	{ //batch_num layer
	    ...
	    thrust::for_each(thrust::make_zip_iterator(
				 thrust::make_tuple(this->dldw_.begin(),
							...
							this->sum_updates_squared_.begin(),
						    _W.begin())),
				 ...
				 thrust::make_tuple(this->dldw_.end(),
							...
							this->sum_updates_squared_.end(),							
						    _W.end())),
			     OptimiserFunctors::AdaDeltaFunctor(this->gamma_, this->offset_));
