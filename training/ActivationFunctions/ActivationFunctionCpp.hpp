...
public:
	...
	ActivationFunctionCpp(std::int32_t _node_number, std::int32_t _dim,
	...
	void calc_output(const std::int32_t _batch_num,
	...
	void calc_dLdw(float *_dLdw, const std::int32_t _batch_num,
			const std::int32_t _batch_size);

	//Virtual function meant to be overridden
	//Adds the bias and applies the activation function during forward propagation
	virtual void forward_propagation(const std::int32_t _batch_size,
			const std::int32_t _dim, const float *_bias,
	...
	virtual void backpropagation(const std::int32_t _batch_size,
			const std::int32_t _dim, thrust::device_vector<float> &_output,
