ActivationFunctionCpp::ActivationFunctionCpp(std::int32_t _node_number,
					     ...
					     RegulariserCpp *_regulariser) : NeuralNetworkNodeCpp(_node_number, _dim, _input_nodes_fed_into_me_dense,
												  _input_nodes_fed_into_me_dense_length,
												  _input_nodes_fed_into_me_sparse,
												  _input_nodes_fed_into_me_sparse_length,
												  _hidden_nodes_fed_into_me, _hidden_nodes_fed_into_me_length,
												  _i_share_weights_with, _no_weight_updates, _regulariser)
{
}
...
void ActivationFunctionCpp::calc_output(const std::int32_t _batch_num,
					const std::int32_t _batch_size)
{
	...
	input_dim = this->neural_net_->get_dense_input_data(i, _batch_num).dim; //For convenience
	...
	cstatus_dense = cublasSgemm(this->neural_net_->get_dense_handle(),			  //handle
				    CUBLAS_OP_T,						  //transa
				    CUBLAS_OP_N,						  //transb
				    _batch_size,						  //m
				    this->dim_,							  //n
				    input_dim,							  //k
				    &alpha,							  //alpha
				    this->neural_net_->get_dense_input_data(i, _batch_num).X_ptr, //A
				    input_dim,							  //lda
				    w,								  //B
				    input_dim,							  //ldb
				    &beta,							  //beta
	...
	if (cstatus_dense != CUBLAS_STATUS_SUCCESS)
		...
		"Something went wrong during cuBLAS operation on dense input data!");
	...
	cstatus_sparse =
	    cusparseScsrmm(
		this->neural_net_->get_sparse_handle(),				       //handle
		CUSPARSE_OPERATION_NON_TRANSPOSE,				       //transA
		_batch_size,							       //m
		this->dim_,							       //n
		input_dim,							       //k
		this->neural_net_->get_sparse_input_data(i, _batch_num).num_non_zero,  //nnz
		&alpha,								       //alpha
		this->neural_net_->get_mat_descr(),				       //descrA
		this->neural_net_->get_sparse_input_data(i, _batch_num).X_data_ptr,    //csrValA
		this->neural_net_->get_sparse_input_data(i, _batch_num).X_indptr_ptr,  //csrRowPtrA
		this->neural_net_->get_sparse_input_data(i, _batch_num).X_indices_ptr, //csrColIndA
		w,								       //B
		input_dim,							       //ldb
		&beta,								       //beta
		this->output_ptr_,						       //C
	...
	if (cstatus_sparse != CUSPARSE_STATUS_SUCCESS)
	    throw std::invalid_argument(
		"Something went wrong during cuSPARSE operation on sparse input data!");
	...
	if (cstatus_dense != CUBLAS_STATUS_SUCCESS)
		...
		"Something went wrong during cuBLAS operation on hidden node data while calculating derivatives!");
    ...
    this->forward_propagation(_batch_size, this->dim_, w, this->output_);
}

void ActivationFunctionCpp::calc_delta(std::int32_t _batch_size)
{

    //Begin by applying the backpropagation functor
    //This multiplies delta by the derivative of the activation function
    this->backpropagation(_batch_size, this->dim_, this->output_, this->delta_);
    ...
    const float *w = this->W_ + num_input_nodes_cumulative_;
	...
	if (cstatus != CUBLAS_STATUS_SUCCESS)
	    throw std::runtime_error(
		"Something went wrong during cuBLAS operation on hidden node data while calculating delta!");

	w += this->dim_ * input_dim; //Increment w to prepare it for next operation
    }
}

void ActivationFunctionCpp::calc_dLdw(float *_dLdw,
				      const std::int32_t _batch_num, const std::int32_t _batch_size)
{
	...
	if (cstatus_dense != CUBLAS_STATUS_SUCCESS)
	    throw std::runtime_error(
		"Something went wrong during cuBLAS operation on input data while calculating derivatives!");
	...
	if (cstatus_sparse != CUSPARSE_STATUS_SUCCESS)
	    throw std::invalid_argument(
		"Something went wrong during cuSPARSE operation while calculating derivatives!");
	...
	thrust::fill(this->ones_.begin(), this->ones_.begin() + _batch_size,
		     1.f);
    }
    ...
    if (cstatus_dense != CUBLAS_STATUS_SUCCESS)
	throw std::runtime_error(
	    "Something went wrong during cuBLAS operation on hidden node data while calculating derivatives for the biases!");

    //Apply regulariser
    this->regulariser_->drdw(this->neural_net_->get_dense_handle(),
			     this->get_num_weights_required(), static_cast<float>(_batch_size),
			     this->W_, _dLdw);
}
