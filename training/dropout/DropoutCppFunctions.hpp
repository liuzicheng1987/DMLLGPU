									  ...
									  nullptr)
{
  ...
  thrust::for_each(
      ...
      thrust::make_counting_iterator(0) + this->num_kernels_,
    ...
    const std::int32_t _batch_size)
{
  ...
  if (this->neural_net_->get_sample() == true)
  {
	...
	this->skip_ = 0;
	  ...
	  thrust::make_counting_iterator(this->skip_) + input_dim * _batch_size,
  ...
  { //If sample is not true
    ...
    for (auto node : this->hidden_nodes_fed_into_me_ptr_)
    {
	  ...
	  thrust::make_constant_iterator(this->dropout_probability_) + input_dim * _batch_size,
	  node->get_output().begin(),
	  this->output_.begin() + output_begin,
...
void DropoutCpp::calc_delta(std::int32_t _batch_size)
{
  ...
  if (this->neural_net_->get_sample() == true)
  {
    ...
    for (auto node : this->hidden_nodes_fed_into_me_ptr_)
    {
	  ...
	  thrust::make_zip_iterator(
	      thrust::make_tuple(
		  this->output_.begin() + output_begin + input_dim * _batch_size,
		  this->delta_.begin() + output_begin + input_dim * _batch_size,
		  node->get_delta().begin() + input_dim * _batch_size)),
