    ...
    const std::int32_t _batch_size)
{
	...
	if (batch_size_input > 0 && join_keys_left[i] >= 0 && join_keys_left[i] < this->input_network_ptr_->get_num_batches())
	{
	    ...
	    //Resize and reset this->included_in_aggregation_, if necessary
		...
		//This is the most important difference between SumCpp and AvgCpp:
		//In SumCpp, this is simply filled with ones, whereas here it is filled
		...
		throw std::runtime_error(
		    "Something went wrong during cuBLAS operation on avg aggregation!");
	}
    }
}

//Calculate the delta of the node (which is used for backpropagation)
void AvgCpp::calc_delta(std::int32_t _batch_size)
{
	...
	if (join_keys_left[i] >= 0 && join_keys_left[i] < this->input_network_ptr_->get_num_batches() && batch_size_input > 0)
	{
	    ...
	    if (cstatus != CUBLAS_STATUS_SUCCESS)
		throw std::runtime_error(
		    "Something went wrong during cuBLAS operation when calculating delta during avg aggregation!");
