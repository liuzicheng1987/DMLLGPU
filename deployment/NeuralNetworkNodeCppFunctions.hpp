NeuralNetworkNodeCpp::NeuralNetworkNodeCpp(
  ...
  this->input_nodes_fed_into_me_dense_ = std::vector<std::int32_t>( _input_nodes_fed_into_me_dense_length);
  for (std::int32_t i=0; i<_input_nodes_fed_into_me_dense_length; ++i) this->input_nodes_fed_into_me_dense_[i] = _input_nodes_fed_into_me_dense[i];    

  //Declare this->input_nodes_fed_into_me_sparse and transfer parameters
  this->input_nodes_fed_into_me_sparse_ = std::vector<std::int32_t>( _input_nodes_fed_into_me_sparse_length);
  for (std::int32_t i=0; i<_input_nodes_fed_into_me_sparse_length; ++i) this->input_nodes_fed_into_me_sparse_[i] = _input_nodes_fed_into_me_sparse[i];    

  //Declare this->hidden_nodes_fed_into_me and transfer parameters
  this->hidden_nodes_fed_into_me_ = std::vector<std::int32_t>(_hidden_nodes_fed_into_me_length);
  for (std::int32_t i=0; i<_hidden_nodes_fed_into_me_length; ++i) this->hidden_nodes_fed_into_me_[i] = _hidden_nodes_fed_into_me[i];    
